...
if (!is_null($badge_photo_path) && !is_null($gov_id_path)) {
    ...
    header('Location: /dashboard/delegates.php?message=' . urlencode('Badge and Gov ID photos updated successfully'));
    die();
    ...
    http_response_code(200);
    header('Location: /dashboard/delegates.php?message=' . urlencode('Badge photo updated'));
    die();
} elseif (is_null($badge_photo_path) && !is_null($gov_id_path)) {
    $stmt = $connection->prepare('UPDATE registrants 
        SET gov_id = ? 
    ...
    http_response_code(200);
    header('Location: /dashboard/delegates.php?message=' . urlencode('Gov ID photo updated successfully'));
    die();
}
